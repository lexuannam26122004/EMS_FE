import { Box, Button, DialogActions, IconButton, InputLabel, Paper, Tooltip, Typography } from '@mui/material'
...
import { IEventGetAll, IFilterEvent, IEventUpdate } from '@/models/Event'
import { renderTimeViewClock } from '@mui/x-date-pickers/timeViewRenderers'
import { toZonedTime, format } from 'date-fns-tz'
import formatDateToTime from '@/utils/formatDateToTime'
import { useSearchEventQuery, useDeleteEventMutation, useUpdateEventMutation } from '@/services/EventService'
...
function Page() {
    ...
    const handleValueChange = (event: SelectChangeEvent<number>) => {
        ...
        setFilter(prev => {
            return {
                ...prev,
                pageNumber: 1,
                isHoliday: value === 1 ? true : value === 2 ? false : null
            }
        })
    }
    ...
    const handleAllDayChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setSelectedEvent(prevEvent => {
            const endOfDay = isAllDay
                ? new Date(prevEvent.end?.setHours(23, 59, 0, 0) ?? prevEvent.start.setHours(23, 59, 0, 0))
                : prevEvent.end
    ...
    const handleSaveEvent = () => {
        ...
        const startTime = convertToVietnamTime(selectedEvent.start)
        let endTime = convertToVietnamTime(selectedEvent.end)
    ...
    useEffect(() => {
        if (isLoadingUpdate === false || isLoadingDelete === false) {
            refetch()
        }
                    ...
                    >
                        ...
                        >
                            <TextField
                                ...
                                {...(isSubmit === true && selectedEvent.title === '' && { error: true })}
                                sx={{
                                    width: '100%',
                                    '& fieldset': {
                                        borderRadius: '8px',
                            ...
                            <Typography
                                sx={{
                                    visibility: isSubmit === true && selectedEvent.title === '' ? 'visible' : 'hidden',
                                    color: 'var(--error-color)',
                                    margin: '1px 0 0 10px',
                                    fontSize: '12px'
                                }}
                        ...
                        >
                            <TextField
                                ...
                                {...(isSubmit === true && selectedEvent.description === '' && { error: true })}
                                multiline
                                rows={3}
                                sx={{
                                    width: '100%',
                            ...
                            <Typography
                                sx={{
                                    visibility:
                                        isSubmit === true && selectedEvent.description === '' ? 'visible' : 'hidden',
                                    color: 'var(--error-color)',
                                    margin: '1px 0 0 10px',
                                    fontSize: '12px'
                                }}
                            ...
                            <Box>
                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                    <DatePicker
                                        ...
                                        onChange={value =>
                                            setSelectedEvent({ ...selectedEvent, start: value?.toDate() || new Date() })
                                        }
                                        sx={{
                                            width: '100%',
                                            '& .MuiInputBase-root': {
                            ...
                            >
                                <LocalizationProvider dateAdapter={AdapterDayjs}>
                                    <DateTimePicker
                                        ...
                                        onChange={value =>
                                            setSelectedEvent({ ...selectedEvent, end: value?.toDate() || new Date() })
                                        }
                                        viewRenderers={{
                                            hours: renderTimeViewClock,
                                            minutes: renderTimeViewClock,
                                ...
                                {colors.map(color => (
                                    <Box key={color}>
                                        ...
                                        >
                                            <CircleIcon
                                                sx={{
                                                    transform: selectedEvent.color === color ? 'scale(1.3)' : 'scale(1)'
                                                }}
                                            />
                ...
                >
                    <Tooltip title={t('COMMON.CALENDAR.DELETE_EVENT')} placement='top'>
                        <IconButton
                            ...
                            onClick={() => {
                                deleteEvent(parseInt(selectedEvent.id))
                                setModalOpen(false)
                            }}
                        >
                            <img
